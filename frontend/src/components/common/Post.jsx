	...
	const {mutate: likePost, isPending: isLiking} = useMutation({
		mutationFn: async () => {
			try {
				...
				if (!res.ok) {
					throw new Error(data.error || "Can't like post")
				}
				return data
			} catch (err) {
				throw new Error(err)
		...
		onSuccess: (updatedLikes) => {
			//not the best way for UX, because it will refetch all posts
	...
	const {mutate: deletePost, isPending: isDeleting} = useMutation({
		...
		mutationFn: async () => {
			try {
				...
				const data = await res.json()
				if (!res.ok) {
					throw new Error(data.error || "Can't delete post")
				}
				return data
			} catch (err) {
				throw new Error(err)
											...
											<div key={comment._id} className='flex gap-2 items-start'>
												<div className='avatar'>
													<div className='w-8 rounded-full'>
														<img
															src={comment.user.profileImg || "/avatar-placeholder.png"} alt={'avatar'}
														/>
													</div>
												</div>
